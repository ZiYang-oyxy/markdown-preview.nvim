...
async function attachIsolatedPreview(page) {
  ...
  await page.evaluate(async ({ token }) => {
    ...
    iframe.contentWindow.postMessage(
      { type: 'mkdp:preview-init', version: 1, token },
      '*',
      [channel.port2],
    )
...
test('raw HTML, event handlers, dangerous URLs, and SVG data never execute', async ({ page }) => {
  ...
  await render(page, `
# 安全测试
<script>window.__previewXss = 'script'</script>
<img src=x onerror="window.__previewXss = 'event'">
![x\" onerror=\"window.__previewXss='alt'](data:image/svg+xml,<svg onload=alert(1)>)
[危险链接](javascript:window.__previewXss='link')
`)

  expect(await frame.evaluate(() => window.__previewXss)).toBeUndefined()
...
graph TD
  A[Safe] --> B[Still safe]
  click A "javascript:window.__previewXss='mermaid'"
\`\`\`

$\\href{javascript:window.__previewXss='katex'}{click}$
