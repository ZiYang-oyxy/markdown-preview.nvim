# Markdown Preview Implementation Plan（已被安全静态版计划取代）

> 后继计划：`2026-07-15-markdown-preview-secure-static.md`。本文件保留用于记录原始设计演进，不再执行。

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build an independent `mkdp preview` workspace that turns pasted Markdown into locally persisted, switchable reading documents with Mermaid rendering on desktop and mobile.

**Architecture:** Add a standalone Preview shell around the existing preview iframe instead of creating a second Markdown renderer. The shell owns IndexedDB persistence, temporary-document navigation, responsive dialogs, and bridge messages; the existing preview page continues to own Markdown-it, Mermaid, KaTeX, TOC, themes, and HTML export.
...
- Documents remain local to browser IndexedDB database `mkdp-markdown-preview`, version `1`.
...
- Root runtime and packaged CLI runtime must expose equivalent Preview behavior.

---

## File Structure

- Create `scripts/lib/markdown-preview-shell.js`: Preview HTML/CSS/client-JS builder and server-testable title helper.
- Modify `scripts/lib/standalone-preview-server.js`: serve the Preview shell and retain Scratch compatibility.
- Modify `scripts/lib/standalone-preview-runtime.js`: start Preview sessions without Browser state.
- Create `scripts/mkdp-preview.js`: repository-local Preview launcher.
- Create `packages/cli/lib/preview-shell.js`: packaged copy of the shell module.
- Modify `packages/cli/lib/server.js` and `packages/cli/lib/runtime.js`: packaged route/runtime parity.
- Create `packages/cli/lib/commands/preview.js`: published CLI command.
- Modify `packages/cli/bin/mkdp.js`: register `mkdp preview`.
- Modify `app/pages/index.jsx`: harden and extend the Preview iframe bridge.
- Modify `package.json` and `README.md` / `README_zh.md`: expose the feature.
- Create `test/markdown-preview-shell.test.js`: helper, shell, route, and runtime tests.
- Create `test/markdown-preview.e2e.test.js`: persistence, switching, editing, Mermaid, and mobile flow.
- Modify `test/cli-package.test.js`: packaged CLI parity.

---

### Task 1: Preview shell contract and route

**Files:**
- Create: `scripts/lib/markdown-preview-shell.js`
- Modify: `scripts/lib/standalone-preview-server.js`
- Test: `test/markdown-preview-shell.test.js`

**Interfaces:**
- Produces: `extractPreviewTitle(markdown, untitledIndex)` and `buildMarkdownPreviewShellHtml(options)`.
...
const {
  buildMarkdownPreviewShellHtml,
  extractPreviewTitle
} = require('../scripts/lib/markdown-preview-shell')

assert.strictEqual(extractPreviewTitle('# AI 工作流\n正文', 1), 'AI 工作流')
assert.strictEqual(extractPreviewTitle('第一行\n第二行', 1), '第一行')
assert.strictEqual(extractPreviewTitle('  \n', 3), '未命名文档 3')

const html = buildMarkdownPreviewShellHtml({ origin: '' })
assert.match(html, /id="preview-shell"/)
...
assert.match(html, /mkdp-markdown-preview/)
assert.match(html, /#1769E8/)
```

- [ ] **Step 2: Run the test to verify it fails**

Run: `node test/markdown-preview-shell.test.js`

Expected: FAIL with `Cannot find module '../scripts/lib/markdown-preview-shell'`.

- [ ] **Step 3: Implement the title helper and semantic shell skeleton**

```js
function extractPreviewTitle(markdown, untitledIndex = 1) {
...
function buildMarkdownPreviewShellHtml() {
  return `<!doctype html>
<html lang="zh-CN">
<head>
  <meta charset="utf-8">
  ...
  <style>
    ...
    * { box-sizing:border-box; }
    ...
    .preview-shell { display:grid;grid-template:58px minmax(0,1fr)/260px minmax(0,1fr) 220px;height:100%; }
    .preview-header { grid-column:1/-1;display:flex;align-items:center;border-bottom:1px solid var(--rule);background:var(--paper); }
    ...
    @media (max-width:767px) { .preview-shell { grid-template:54px minmax(0,1fr)/1fr; } #toc-rail { display:none; } }
  </style>
</head>
<body>
  <div id="preview-shell" class="preview-shell">
    ...
    <aside id="toc-rail" aria-label="本页目录"><h2>本页目录</h2><nav id="preview-toc"></nav></aside>
  </div>
...
module.exports = { buildMarkdownPreviewShellHtml, extractPreviewTitle }
```
...
- [ ] **Step 4: Register Preview and Scratch-compatible routes**

```js
const { buildMarkdownPreviewShellHtml } = require('./markdown-preview-shell')

if (
  pathname === '/_mkdp/preview' || pathname === '/_mkdp/preview/' ||
  pathname === '/_mkdp/scratch' || pathname === '/_mkdp/scratch/'
) {
  res.statusCode = 200
  res.setHeader('content-type', 'text/html; charset=utf-8')
  res.end(buildMarkdownPreviewShellHtml())
  return
}
```

Remove `buildScratchShellHtml` only after all imports/tests use the new builder; export `buildMarkdownPreviewShellHtml` from the server for compatibility tests.

- [ ] **Step 5: Add a route test and run it**

Extend `test/markdown-preview-shell.test.js` to start `startStandalonePreviewServer` without `browseRoot`, request both routes, and assert status `200` plus `id="preview-shell"`.

Run: `node test/markdown-preview-shell.test.js`

Expected: PASS and `markdown-preview-shell tests: ok`.

- [ ] **Step 6: Commit**

```bash
git add scripts/lib/markdown-preview-shell.js scripts/lib/standalone-preview-server.js test/markdown-preview-shell.test.js
git commit -m "feat(preview): add standalone workspace shell"
```
...
**Files:**
- Modify: `scripts/lib/markdown-preview-shell.js`
- Modify: `test/markdown-preview-shell.test.js`
- Create: `test/markdown-preview.e2e.test.js`

**Interfaces:**
- Produces client functions `openPreviewDatabase`, `listDocuments`, `putDocument`, `deleteDocument`, `createDocumentFromMarkdown`, and `selectDocument` inside the shell.
...
```js
await page.goto(`${server.origin}/_mkdp/preview`)
...
Run: `node test/markdown-preview.e2e.test.js`
...
```js
var DB_NAME = 'mkdp-markdown-preview'
var DB_VERSION = 1
...
function createDocumentFromMarkdown(markdown) {
  ...
  var documentRecord = {
    id: createPreviewId(),
    title: extractPreviewTitle(markdown, state.documents.length + 1),
    titleMode: 'auto',
    markdown: markdown,
    createdAt: now,
    updatedAt: now
...
```bash
git add scripts/lib/markdown-preview-shell.js test/markdown-preview-shell.test.js test/markdown-preview.e2e.test.js
git commit -m "feat(preview): persist pasted markdown documents"
```
...
- Modify: `test/markdown-preview.e2e.test.js`
...
Retain same-origin operation for Browser and Preview and add a preview test proving a foreign-origin synthetic message is ignored.
...
- Render preview `tocItems` messages into `#preview-toc` and send `mkdp:scroll-to` on click.
...
- [ ] **Step 6: Run focused preview and Preview tests**
...
```bash
git add scripts/lib/markdown-preview-shell.js app/pages/index.jsx app/out dist/web test/markdown-preview.e2e.test.js
git commit -m "feat(preview): add reading and source tools"
```
...
Run: `node test/markdown-preview.e2e.test.js --mobile`
...
Run the Preview server and capture:

- Desktop: `1536x1024` to `test/artifacts/markdown-preview-desktop.png`.
- Mobile reading: `390x844` to `test/artifacts/markdown-preview-mobile-reading.png`.
- Mobile paste: `390x844` to `test/artifacts/markdown-preview-mobile-paste.png`.

Inspect all three against `docs/superpowers/assets/markdown-preview/desktop-concept.png` and `mobile-concept.png`; fix layout, typography, palette, overflow, and control-density drift before proceeding.
...
```bash
git add scripts/lib/markdown-preview-shell.js test/markdown-preview.e2e.test.js test/artifacts/markdown-preview-*.png
git commit -m "style(preview): implement responsive reading workspace"
```
...
- Modify: `package.json`
- Create: `packages/cli/lib/preview-shell.js`
...
**Interfaces:**
- Produces: `createStandalonePreviewSession(cliConfig)` in both runtimes.
- Produces: `npm run preview` and `mkdp preview [options]`.

- [ ] **Step 1: Write failing runtime and CLI tests**

```js
assert.match(help.stdout, /preview\s+Open the temporary Markdown workspace/)

const { createStandalonePreviewSession } = require('../packages/cli/lib/runtime')
const session = await createStandalonePreviewSession({ theme: 'light' })
const response = await requestText(`${session.origin}/_mkdp/preview`)
assert.strictEqual(response.statusCode, 200)
assert.match(response.body, /id="preview-shell"/)
await session.close()
```
...
Expected: FAIL because `preview` and `createStandalonePreviewSession` are absent.

- [ ] **Step 3: Add the root Preview session and launcher**

```js
async function createStandalonePreviewSession(cliConfig = {}) {
...
`scripts/mkdp-preview.js` follows the existing browse launcher but opens `/_mkdp/preview`. Add `"preview": "node ./scripts/mkdp-preview.js"` to root scripts.

- [ ] **Step 4: Add packaged CLI parity**

Copy the completed shell module to `packages/cli/lib/preview-shell.js`, wire the same route into `packages/cli/lib/server.js`, add the runtime function, and create `commands/preview.js` with options `--config`, `--theme`, `--page-title`, `--markdown-css`, `--highlight-css`, `--images-path`, and `--browser`.
...
```bash
git add scripts/lib/standalone-preview-runtime.js scripts/mkdp-preview.js package.json packages/cli/lib/preview-shell.js packages/cli/lib/server.js packages/cli/lib/runtime.js packages/cli/lib/commands/preview.js packages/cli/bin/mkdp.js test/cli-package.test.js
git commit -m "feat(cli): add markdown preview command"
```
...
- Modify: `README.md`
- Modify: `README_zh.md`
- Modify: `docs/superpowers/specs/2026-07-15-markdown-preview-design.md`
...
```bash
node test/markdown-preview-shell.test.js
node test/markdown-preview.e2e.test.js
...
```bash
git add README.md README_zh.md docs/superpowers/specs/2026-07-15-markdown-preview-design.md
git commit -m "docs(preview): document temporary markdown workspace"
```
