export const INDEX_KEY = 'mkdp-studio:index:v1'
export const PREFERENCES_KEY = 'mkdp-studio:preferences:v1'
export const DOCUMENT_KEY_PREFIX = 'mkdp-studio:document:'
...
const STUDIO_KEY_PREFIX = 'mkdp-studio:'
...
export function createStudioStorage({
  ...
  function subscribeToStorage(listener) {
    const onStorage = (event) => {
      if (event.key === null || event.key?.startsWith(STUDIO_KEY_PREFIX)) listener()
    }
...
function getBrowserStorage() {
  browserStorage ??= createStudioStorage()
