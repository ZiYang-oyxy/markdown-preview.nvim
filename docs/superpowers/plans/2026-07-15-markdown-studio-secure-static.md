...
- Create: `studio/public/studio-preview.html`
- Create: `studio/vite.config.js`
- Create: `studio/vite.preview.config.js`
- Create: `studio/src/main.jsx`
- Create: `studio/src/storage.js`
- Create: `studio/src/storage.test.js`
...
- Produces static `studio/dist/index.html` and `studio/dist/studio-preview.html`.
...
Generate `package-lock.json` with `npm install` inside `studio/`; do not edit lockfile manually.
...
```js
const store = createStudioStorage(memoryStorage())
...
Run: `npm test -- storage.test.js` from `studio/`.

Expected: FAIL because `storage.js` does not exist.

- [ ] **Step 4: Implement constrained localStorage**

Use keys `mkdp-studio:index:v1`, `mkdp-studio:document:<id>`, and `mkdp-studio:preferences:v1`. Compute UTF-8 bytes with `new TextEncoder().encode(value).byteLength`; write the document before metadata, roll back the document if metadata fails, and never report saved state after an exception. Use `crypto.randomUUID()` with a random-value fallback.

- [ ] **Step 5: Create the React shell skeleton and relative static build**

`vite.config.js` uses `base:'./'`. `vite.preview.config.js` builds `src/preview/entry.js` as a stable classic IIFE file `dist/assets/studio-preview.js` with `emptyOutDir:false`; `public/studio-preview.html` loads that file with a classic `<script defer>` so it can run inside an opaque sandbox without module CORS.
...
Run: `npm test` and `npm run build` from `studio/`.
...
```bash
git add studio
git commit -m "feat(studio): scaffold secure static workspace"
```
...
**Files:**
- Create: `studio/src/protocol.js`
- Create: `studio/src/protocol.test.js`
- Create: `studio/src/preview/entry.js`
- Create: `studio/src/preview/render-markdown.js`
- Create: `studio/src/preview/render-markdown.test.js`
- Create: `studio/src/preview/preview.css`
- Create: `studio/playwright.config.js`
- Create: `studio/e2e/security.spec.js`

**Interfaces:**
- Shell produces `createPreviewChannel(iframe, handlers)` with `render`, `scrollTo`, and `requestSnapshot` methods.
- Preview accepts one `mkdp:studio-init` from `window.parent`, then communicates only through transferred `MessagePort`.
...
Run: `npm run test:e2e -- security.spec.js` from `studio/`.
...
Parent verifies the load event belongs to its iframe element, creates a random token and `MessageChannel`, then sends `{type:'mkdp:studio-init',version:1,token}` to `iframe.contentWindow` with target `'*'` and transfers `port2`. Preview accepts exactly one message where `event.source===window.parent`, validates shape, removes the global listener, stores the token, and starts `port.start()`.
...
Expected: all pass; built `studio-preview.html` contains `sandbox` only on the parent iframe, and no security payload executes.

- [ ] **Step 8: Commit**

```bash
git add studio
git commit -m "feat(studio): add sandboxed safe markdown renderer"
```
...
**Files:**
- Create: `studio/src/App.jsx`
- Create: `studio/src/components/AppHeader.jsx`
- Create: `studio/src/components/DocumentRail.jsx`
- Create: `studio/src/components/DocumentToolbar.jsx`
- Create: `studio/src/components/PasteDialog.jsx`
- Create: `studio/src/components/SourceDialog.jsx`
- Create: `studio/src/components/TocRail.jsx`
- Create: `studio/src/components/Sheet.jsx`
- Create: `studio/src/components/MermaidViewer.jsx`
- Modify: `studio/src/main.jsx`
- Modify: `studio/src/styles.css`
- Create: `studio/e2e/workflow.spec.js`
- Create: `studio/e2e/mobile.spec.js`
...
```bash
git add studio
git commit -m "feat(studio): implement paste-first reading workspace"
```
...
**Files:**
- Create: `studio/src/export.js`
- Create: `studio/src/export.test.js`
- Create: `studio/e2e/export-security.spec.js`
- Create: `scripts/lib/studio-static-server.js`
- Create: `scripts/mkdp-studio.js`
- Modify: `package.json`
- Create: `test/studio-static-server.test.js`
- Create: `packages/cli/lib/studio-static-server.js`
...
- `startStudioStaticServer({port,assetRoot})` returns `{origin,url,close}`.
...
```bash
git add studio scripts package.json packages/cli test
git commit -m "feat(cli): serve secure static markdown studio"
```
...
**Files:**
- Create: `studio/deploy/nginx.conf.example`
- Create: `studio/deploy/_headers`
- Modify: `README.md`
- Modify: `README_zh.md`
...
From `studio/`: `npm audit --audit-level=high`, `npm test`, `npm run build`, and the full Playwright suite.
...
```bash
git add studio README.md README_zh.md docs
git commit -m "docs(studio): add secure static deployment guide"
```
