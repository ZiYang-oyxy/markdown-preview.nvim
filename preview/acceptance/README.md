...
python [REDACTED_LOCAL_VALUE]/.agents/skills/webapp-testing/scripts/with_server.py \
  --server "npm --prefix preview run preview -- --host 127.0.0.1 --port 4175 --strictPort" \
  --port 4175 \
  --timeout 30 \
  -- python -u preview/acceptance/acceptance.py
```

脚本退出码非零代表验收失败。结果和截图生成在 `preview/acceptance/artifacts/`，该目录不进入 Git。
...
npm --prefix preview audit --audit-level=high
```
