# Markdown Preview 临时 Markdown 工作台设计

- **日期**：2026-07-15
- **状态**：已被安全静态版规格取代
- **后继规格**：`2026-07-15-markdown-preview-secure-static-design.md`
- **定位**：从 Browser 中独立出来的粘贴式 Markdown 阅读工具

## 1. 目标

Markdown Preview 面向“复制 AI 输出并立即阅读”的场景。用户粘贴 Markdown 源文本后，系统创建一份新的临时文档，立即使用现有 Markdown 预览管线渲染 HTML，并在浏览器本地持久化。产品默认提供完整阅读体验，源码编辑只在用户主动打开时出现。
...
- 作为独立 `mkdp preview` 功能运行，不要求先进入 Browser。
...
| 独立入口 | `mkdp preview` 启动，主路由 `/_mkdp/preview` |
| 兼容入口 | 原 `/_mkdp/scratch` 保留为 Preview 别名或重定向 |

## 4. 视觉设计

### 4.1 桌面端

![Markdown Preview 桌面端设计](../assets/markdown-preview/desktop-concept.png)
...
![Markdown Preview 移动端设计](../assets/markdown-preview/mobile-concept.png)
...
实现继续使用同源 iframe 隔离渲染页面。Preview Shell 管理文档、导航和响应式交互；现有 `app/pages/index.jsx` 继续负责 Markdown 渲染。二者通过已有 `mkdp:set-content`、主题、导出和 TOC 消息通信，避免出现第二套渲染行为。

### 6.1 模块边界

| 模块 | 职责 |
| --- | --- |
| `markdown-preview-shell.js` | 生成 Preview HTML shell，承载布局、交互和 IndexedDB 客户端逻辑 |
| `standalone-preview-server.js` | 暴露 Preview 路由和预览资源，不管理文档内容 |
| `standalone-preview-runtime.js` | 创建无 Browser 依赖的 Preview session |
| `mkdp-preview.js` / CLI command | 启动服务、打开 `/_mkdp/preview`、处理退出 |
| `app/pages/index.jsx` | 复用现有 Markdown/Mermaid/KaTeX 渲染和导出 |

### 6.2 IndexedDB 数据模型

数据库名：`mkdp-markdown-preview`，版本：`1`。
...
- Preview shell 包含所需 landmark、dialog、iframe 和持久化入口。
- `/_mkdp/preview` 在未启用 `browseRoot` 时返回 200。
- `/_mkdp/scratch` 仍可访问并进入 Preview。
- 标题提取覆盖 H1、首行和空标题回退。
- CLI 帮助包含 `mkdp preview`，命令可启动并退出。
...
- Browser 的文件浏览行为保持不变；其原 Scratch 入口指向新的 Preview。
- 根仓库脚本和发布 CLI 包都提供 `preview` 命令。
