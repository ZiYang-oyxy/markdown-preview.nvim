...
For public or intranet deployment, publish **only** `preview/dist/` at the root of a **dedicated origin**. Do not mount it under an existing application's origin and do not expose the legacy dynamic server as the Markdown Preview backend. See [`preview/README.md`](preview/README.md) for the security model, deployment headers, storage limits, and verification commands.
...
Click the pencil button in the browse sidebar header (or open `/_mkdp/scratch` directly) to paste or type Markdown into a left-hand editor and see it rendered live on the right. It reuses the full preview pipeline (mermaid, KaTeX, syntax highlighting) and supports theme switching and standalone HTML export from the top bar — handy for quickly rendering snippets without saving a file.
