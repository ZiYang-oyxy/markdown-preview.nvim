...
const { startPreviewStaticServer } = require('../preview-static-server')
...
async function run(argv, runtimeOptions = {}) {
  ...
  const assetRoot = runtimeOptions.assetRoot || path.join(__dirname, '..', '..', 'assets', 'preview')
  const session = await startPreviewStaticServer({ assetRoot, port: options.port })
