...
验收对象：`feature/markdown-preview` 生产构建
...
`preview/index.html:5` - viewport 未禁用缩放；color-scheme、theme-color 与 CSP 已配置，pass

### `preview/src/App.jsx`

`preview/src/App.jsx:178` - skip link 指向唯一 main landmark，键盘跳转已通过黑盒验证

`preview/src/App.jsx:226` - 隐藏文件输入具备可访问名称、`name` 与 `autocomplete`，pass

### `preview/src/components/PasteDialog.jsx`

`preview/src/components/PasteDialog.jsx:72` - textarea 具备 label、name、autocomplete、spellcheck 与 inline error，pass

### `preview/src/components/SourceDialog.jsx`

`preview/src/components/SourceDialog.jsx:62` - textarea 具备 label、name、autocomplete 与明确保存文案，pass

### `preview/src/styles.css`

`preview/src/styles.css:56` - button 使用 `touch-action: manipulation` 与明确 tap highlight，pass

`preview/src/styles.css:65` - 键盘 focus-visible 样式存在，pass

`preview/src/styles.css:614` - modal/Sheet 限制 overscroll，pass

`preview/src/styles.css:744` - rail 收起时平板导航入口保持可见，pass
...
截图位于 `preview/acceptance/artifacts/`：
