...
describe('Markdown Preview protocol validation', () => {
  it('accepts only an exact versioned initialization message', () => {
    expect(
      validateInitMessage({ type: 'mkdp:preview-init', version: 1, token }),
    ).toEqual({ ok: true })
    expect(
      validateInitMessage({ type: 'mkdp:preview-init', version: 2, token }),
    ).toMatchObject({ ok: false })
    expect(
      validateInitMessage({ type: 'mkdp:preview-init', version: 1, token, extra: true }),
    ).toMatchObject({ ok: false })
  })
