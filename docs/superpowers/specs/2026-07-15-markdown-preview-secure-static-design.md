...
- **日期**：2026-07-15
- **状态**：已确认，待实现
- **取代**：`2026-07-15-markdown-preview-design.md`

> **命名说明（2026-07-15）**：用户可见产品名改为 **Markdown Preview**，主 CLI 入口改为 `mkdp paste`。`mkdp preview` 暂时作为隐藏兼容别名；内部 `preview` 路径、`mkdp-preview:*` 存储 key 与 `mkdp:preview-init` 协议名暂不迁移。
...
`preview/dist/` 可直接部署到 Nginx、对象存储、GitHub Pages 或 CDN。它只包含静态 HTML、CSS、JS、字体和图标，不包含 API。

### 3.2 本地 CLI

`mkdp paste` 使用最小静态文件服务器提供相同的 `preview/dist/`：
...
| Key | 内容 |
| --- | --- |
| `mkdp-preview:index:v1` | 文档 metadata、排序、当前文档 ID、schema version |
| `mkdp-preview:document:<id>` | 单份 Markdown 源文本 |
| `mkdp-preview:preferences:v1` | 主题和界面偏好 |
...
3. 父页面仅向该 `contentWindow` 发送 `mkdp:preview-init`，并 transfer `port2`。
...
- [桌面端设计稿](../assets/markdown-preview/desktop-concept.png)
- [移动端设计稿](../assets/markdown-preview/mobile-concept.png)
