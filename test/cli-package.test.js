...
async function testBrowseShellUsesCurrentToolboxUi() {
  ...
  try {
    ...
    // 防止只打包旧版 browse shell：以下为 master 最新功能标记
    assert.match(response.body, /id="scratch-link"/)
    assert.match(response.body, /id="toc-sidebar"/)
    assert.match(response.body, /class="search-spinner"/)
  } finally {
    if (session) {
      await session.close()
    }
