...
function startStudioStaticServer(options = {}) {
  const assetRoot = fs.realpathSync(path.resolve(options.assetRoot || path.join(__dirname, '..', 'assets', 'studio')))
  ...
  const server = http.createServer((request, response) => {
    ...
    fs.realpath(filePath, (realPathError, realPath) => {
      ...
      fs.readFile(realPath, (readError, content) => {
        ...
        if (extension === '.html') {
          headers['Content-Security-Policy'] = path.basename(realPath) === 'studio-preview.html'
            ? PREVIEW_CSP
            : SHELL_CSP
        }
