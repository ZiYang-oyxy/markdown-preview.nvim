...
const { startPreviewStaticServer } = require('../scripts/lib/preview-static-server')
...
async function main() {
  const assetRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'mkdp-preview-static-'))
  fs.mkdirSync(path.join(assetRoot, 'assets'))
  fs.writeFileSync(path.join(assetRoot, 'index.html'), '<h1>Markdown Preview shell</h1>')
  fs.writeFileSync(path.join(assetRoot, 'preview-frame.html'), '<h1>Preview</h1>')
  fs.writeFileSync(path.join(assetRoot, 'assets', 'app.js'), 'console.log("safe")')
  let session

  try {
    session = await startPreviewStaticServer({ assetRoot, port: 0 })
    ...
    const preview = await request(`${session.url}preview-frame.html`)
  ...
  process.stdout.write('preview static server tests: ok\n')
}
