...
get_latest_release() {
  fetch "https://api.github.com/repos/ZiYang-oyxy/markdown-preview/releases/latest" |
    grep '"tag_name":' |
    sed -E 's/.*"([^"]+)".*/\1/'
}
...
download() {
  mkdir -p bin
  cd bin
  url="https://github.com/ZiYang-oyxy/markdown-preview/releases/download/$tag/${1}"
